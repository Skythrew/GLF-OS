...
install:
	...
	if [ -n "$ISO_FILE" ]; then
		...
		cp "$ISO_FILE" "GLF-OS-ALPHA-OMNISLASH_2.iso" # Copie avec la nomination GLF OS
		sha256sum "GLF-OS-ALPHA-OMNISLASH_2.iso" > "GLF-OS-ALPHA-OMNISLASH_2.iso.sha256sum"
		cat "GLF-OS-ALPHA-OMNISLASH_2.iso.sha256sum"
	else
