...
install:
	...
	if [ -n "$ISO_FILE" ]; then
		...
		cp "$ISO_FILE" "GLF-OS-ALPHA-STABLE_final.iso" # Copie avec la nomination GLF OS
		sha256sum "GLF-OS-ALPHA-OMNISLASH_3.iso" > "GLF-OS-ALPHA-STABLE_final.iso.sha256sum"
		cat "GLF-OS-ALPHA-STABLE_final.iso.sha256sum"
	else
